...
int main (int argc, char** argv) {
  ...
  initializeGame(num_of_players, cards, atoi(argv[1]),
        &numHandCards(struct gameState *game_state_1));
  initializeGame(num_of_players, cards, atoi(argv[1]),
        &numHandCards(struct gameState *game_state_2));
  initializeGame(num_of_players, cards, atoi(argv[1]),
        &numHandCards(struct gameState *game_state_3));

  //Test for choice 1
  initial_num_cards = numHandCards(game_state_1);
  cardEffect(steward, 1, 0, 0, game_state_1, 0, 0);
  assert(numHandCards(game_state_1) == initial_num_cards + 1
        && "Steward drew the wrong number of cards\n");

  //Test for choice 2
  initial_num_coin = game_state_2->coins;
  cardEffect(steward, 2, 0, 0, game_state_2, 0, 0);
  assert(game_state_2->coins == initial_num_coin + 2
  ...
  assert(numHandCards(game_state_3) == initial_num_cards - 3
